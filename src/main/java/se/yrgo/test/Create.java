...
public class Create {
    ...
    public static void main(String[] args) {
        ...
        // setUp(em); //Do setUp first and then the task's. OBS!!! persistence.xml
        // (hibernate.hbm2ddl.auto has the value "create")
    ...
    // Get all books by a specific author (JPQL)
    public static void task2WithQuery(EntityManager em) {
        Author theAuthor = em.find(Author.class, 1); //J.K. Rowling 
        ...
        List<Book> theAuthorsBooks = em.createQuery("select author.books from Author as author where author.name = :name")
        .setParameter("name", theAuthor.getName()).getResultList();
    ...
    public static void task6(EntityManager em) {
        ...
        for (Object[] obj : authorAndBook) {
            ...
            System.out.println("Author: " + theAuthor.getName() + " (id: " + theAuthor.getId() + "), " + "Booktitle: "
                    + theBook.getTitle());
        }
    }

    public static void task6ButEasyer(EntityManager em) {
        String chooseGenre = "Fantasy";

        List<Book> authorAndBook = em.createNamedQuery("searchByGenre")
                .setParameter("genre", chooseGenre)
                .getResultList();

        System.out.println("chosen genre: " + chooseGenre);
        
        for (Book obj : authorAndBook) {
            System.out.println("(Book id: " + obj.getId() + "), " + "Booktitle: "+ obj.getTitle());
        }
    }
}